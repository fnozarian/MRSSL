...
import com.aut.ceit.clustering.mrssl.spatial.BoundsInOneDimension._
import com.aut.ceit.clustering.mrssl.{SuiteBase, MRSSLSettings}
...
class BoxCalculatorSuite extends SuiteBase {
  ...
  test ("BoxCalculator should generate 2-level hierarchy of boxes") {
    ...
    val mrsslSettings = new MRSSLSettings ().withEpsilon(1)
    ...
    val boxTreeRoot = BoxCalculator.generateTreeOfBoxes(rootBox, partitioningSettings, mrsslSettings)
  ...
  test("BoxCalculator should count points in each partition") {
    ...
    val root1 = BoxCalculator.generateTreeOfBoxes(rootBox, partitioningSettings, mrsslSettings)
    val root2 = BoxCalculator.generateTreeOfBoxes(rootBox, partitioningSettings, mrsslSettings)
  ...
  test("BoxCalculator should split a box into 6 boxes") {
    ...
    val settings = new MRSSLSettings ().withEpsilon(0.5)
