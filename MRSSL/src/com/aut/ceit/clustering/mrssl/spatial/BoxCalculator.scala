...
import com.aut.ceit.clustering.mrssl.spatial.rdd.{ BoxPartitioner, PartitioningSettings }
import com.aut.ceit.clustering.mrssl._
import org.apache.spark.rdd.RDD
import org.apache.spark.SparkContext._

/**
 * Calculates box-shaped regions for density-based partitioning (see [[com.aut.ceit.clustering.mrssl.spatial.rdd.BoxPartitioner]] )
 * and for fast lookup of point's neighbors (see [[com.aut.ceit.clustering.mrssl.spatial.PartitionIndex]]
 *
...
class BoxCalculator(val data: RawDataSet) {
  ...
  def generateDensityBasedBoxes(partitioningSettings: PartitioningSettings = new PartitioningSettings(),
                                mrsslSettings: MRSSLSettings = new MRSSLSettings()): (Iterable[Box], Box) = {

    val datasetBounds = calculateBounds(data, numberOfDimensions)
    val rootBox = new Box(datasetBounds.toArray)
    val boxTree = BoxCalculator.generateTreeOfBoxes(rootBox, partitioningSettings, mrsslSettings)
  ...
  private[mrssl] def getNumberOfDimensions(ds: RawDataSet): Int = {
    val pt = ds.first()
  ...
  private[mrssl] def calculateBounds(ds: RawDataSet, dimensions: Int): List[BoundsInOneDimension] = {
...
private object BoxCalculator {
  ...
  def generateTreeOfBoxes(root: Box,
                          ...
                          mrsslSettings: MRSSLSettings): BoxTreeItemWithNumberOfPoints = {
    BoxCalculator.generateTreeOfBoxes(root, partitioningSettings, mrsslSettings, new BoxIdGenerator(root.boxId))
  }

  def generateTreeOfBoxes(root: Box,
                          partitioningSettings: PartitioningSettings,
                          mrsslSettings: MRSSLSettings,
    ...
    result.children = if (partitioningSettings.numberOfLevels > 0) {
      ...
      root
        ...
        .filter(_.isBigEnough(mrsslSettings))
  ...
  private[mrssl] def generateCombinationsOfSplits(splits: List[List[BoundsInOneDimension]],
  ...
  def splitBoxIntoEqualBoxes(rootBox: Box, maxSplits: Int, mrsslSettings: MRSSLSettings): Iterable[Box] = {

    val dimensions = rootBox.bounds.size
    val splits = rootBox.bounds.map(_.split(maxSplits, mrsslSettings))
  ...
  private[mrssl] def assignAdjacentBoxes(boxesWithEnoughPoints: Iterable[Box]) = {
  ...
  private[mrssl] def generateDistinctPairsOfAdjacentBoxIds(boxesWithAdjacentBoxes: Iterable[Box]): Iterable[PairOfAdjacentBoxIds] = {
  ...
  private[mrssl] def shouldAdjacentBoxBeIncludedInPartition(rootBoxId: BoxId, adjacentBoxId: BoxId): Boolean = {
    rootBoxId <= adjacentBoxId
  }

  private[mrssl] def generateEmbracingBox(boxes: Iterable[Box]): Box = {
  ...
  private[mrssl] def generateEmbracingBoxFromAdjacentBoxes(rootBox: Box): Box = {
