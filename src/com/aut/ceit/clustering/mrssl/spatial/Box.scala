...
import com.aut.ceit.clustering.mrssl.{MRSSLSettings, BoxId}
...
/** Represents a box-shaped region in multi-dimensional space
  ...
  */
private [mrssl] class Box (val bounds: Array[BoundsInOneDimension], val boxId: BoxId = 0, val partitionId: Int = -1, var adjacentBoxes: List[Box] = Nil)
  ...
  def isBigEnough (settings: MRSSLSettings): Boolean = {

    val bLengths = bounds.map { x => x.length.toString }.mkString(",")
  ...
  private [mrssl] def findLongestDimensionAndItsIndex ()
  ...
  private [mrssl] def calculateCenter (b: Array[BoundsInOneDimension]): Point = {
